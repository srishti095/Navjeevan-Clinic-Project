...
const STATUS_STYLES: Record<string, string> = {
  period: 'bg-brand-500 text-white',
  fertile: 'bg-brand-100 text-brand-700',
  ovulation: 'bg-purple-500 text-white',
  predicted: 'bg-sky-100 text-sky-700 border border-sky-300 border-dashed',
};

const LEGEND = [
  { label: 'Period', cls: 'bg-brand-500' },
  { label: 'Fertile', cls: 'bg-brand-100' },
  { label: 'Ovulation', cls: 'bg-purple-500' },
  { label: 'Predicted', cls: 'bg-sky-100 border border-sky-300 border-dashed' },
];
...
export default function PeriodCalendar({
  ...
  getDayStatus: (d: Date) => CalendarDayStatus;
}) {
    ...
    <div className="bg-white rounded-2xl p-6 border border-brand-100">
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((d, i) => {
          ...
          const base = 'aspect-square flex items-center justify-center rounded-lg text-sm transition';
